    ...
    try {
        ...
        const connections = user.connections
            ...
            .map(c => ({
                ...
                avatar: c.user.avatar,
                bio: c.user.bio,
                connectedAt: c._id.getTimestamp() 
            }));
...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        const excludeIds = new Set([
            ...currentUser.connections.map(c => c.user._id.toString()),
            currentUserId
        ]);
        ...
        .select('name username avatar bio connections interestVector solvedProblems')
        .populate('connections.user', 'name avatar')
        .lean();
        ...
        const suggestions = candidates.map(candidate => {
            ...
            if (sharedInterests > 0) score += (sharedInterests * 3);

            score += (candidate.solvedProblems?.length || 0) * 0.1;
            ...
            return {
                ...
                reason: mutualUsers.length > 0 
                    ? (sharedInterests > 0 ? "Mutuals & Interests" : "Mutual connections")
